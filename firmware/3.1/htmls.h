...
<div id="menuDropDownB">Backup</div>
<div id="menuDropDownR">Restore</div>
<!--<div>Setting</div>-->
</div>
</div>
</div>
...
w('<button class="changeCodeBtn underScoreBtn">Set Code</button>');
...
Connection = { Connected:"Connected", Disconnected:"Disconnected", Timeout:"Timeout", Connecting:"Connecting",Busy:"Busy"}
...
// JRN - Backup option
let backup = id("menuDropDownB");
backup.onclick = function() {
id("menuDropDownContent").classList.toggle("menuDropDownShow");
changeStatus(Connection.Busy);
fetch(ip+'jc')
...
a.href = url;
a.download =  "rftoy-backup-" + new Date().toLocaleDateString().replace( /\//g, "-" ) + ".json";
a.click();
...
changeStatus(Connection.Busy);
let xhr = new XMLHttpRequest();
let url = ip+"cc?sid="+i+"&name="+encodeURIComponent(data.stations[i].name)+"&code="+encodeURIComponent(data.stations[i].code);
xhr.open("GET", url, true);
//xhr.timeout = MAX_TIMEOUT;
...
// Change code button
let cco = clas("changeCodeBtn");
cco[i].onclick = function(){
let btn = cco[i].parentNode.previousSibling.firstChild;
let input = prompt("Enter the hexadecimal code for "+btn.innerHTML+" [First 6 ON, next 6 OFF, last 4 timing (recommend 0158)]",
"00553300553c0158");
if (input != null) {
if (input.length != 16) {
...
let xhr = new XMLHttpRequest();
let url = ip+"cc?sid="+i+"&code="+encodeURIComponent(input);
xhr.open("GET", url, true);
//xhr.timeout = MAX_TIMEOUT;
//xhr.ontimeout = timeoutProcedure;
xhr.onreadystatechange = function(){
...
let rfBtn = cco[i].nextElementSibling;
